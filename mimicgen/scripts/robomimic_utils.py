...
copied from josiah'c code base
"""
import copy
...
def get_epochs_trained(checkpoint_name):
    try:
        start_index = checkpoint_name.index("epoch_") + len("epoch_")
        epoch_str = ""
        for char in checkpoint_name[start_index:]:
            if char.isdigit():
                epoch_str += char
            else:
                break
        ...
        print("Error: Unable to extract epoch number from checkpoint name.")
        return None

def get_latest_ckpt_path(command_string, logdir_name):
    # Find the start index of the file name by searching for "--config " and adding its length
    start_index = command_string.find("--config ") + len("--config ")

    # Find the end index of the file name by searching for ".json"
    end_index = command_string.find(".json")

    # Extract the substring containing the file name
    config_name = os.path.basename(command_string[start_index : end_index])

    doppelmaker_path = '/'.join(doppelmaker.__path__[0].split('/')[:-1])
    output_path = f"{doppelmaker_path}/open_or_close_skill/{logdir_name}"
    output_path = os.path.join(output_path, config_name)
    latest_timestamp = sorted(os.listdir(output_path))[-1]
    output_path = os.path.join(output_path, latest_timestamp)    
    all_ckpt_dir = os.path.join(output_path, "models")

    ckpt_names = os.listdir(all_ckpt_dir)
    latest_ckpt_name = None
    latest_epoch = -1
    for fname in ckpt_names:
        epoch = get_epochs_trained(fname)
        if epoch > latest_epoch:
            latest_ckpt_name = fname
            latest_epoch = epoch
    latest_ckpt_path = os.path.join(all_ckpt_dir, latest_ckpt_name)
    print("Found latest_ckpt_path: ", latest_ckpt_path)
...
def get_env_all_objects_initialization_from_dataset(dataset_path, ds_format="robomimic"):
    """
    ...
    if ds_format == "robomimic":
        if "env_other_objects" in f["data"].attrs:
            env_all_obj_meta = json.loads(f["data"].attrs["env_other_objects"])
        else:
            return None
    # elif ds_format == "r2d2":
...
def run_rollout(
    ...
    try:
        save_video = {'external::external_cam::depth': f'/scr/tydai/doppelmaker/open_or_close_skill/video_depth/external_cam_depth_{video_count}.mp4', \
    ...
    def _forward_in(self, inputs):
        ...
        if self.downsampling_enabled:
            if self.use_fps_downsampling:
                ...
                idxs_b = np.stack([fpsample.bucket_fps_kdline_sampling(pc_np[i][:3], self.downsampling_n, h=7) for i in range(BNS)], axis=0).astype(int)
