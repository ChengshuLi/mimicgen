...
def main():
    ...
    for chair in env.scene.object_registry("category", "taboret"):
        chair.visible = False
    ...
    def save_video(writer):
        obs, obs_info = og.sim.viewer_camera.get_obs()
        rgb_numpy = obs["rgb"][:, :, :3].cpu().numpy()
        writer.append_data(rgb_numpy)
    ...
    def save_timestep_from_to(timestep_from, timestep_to, video_file, fps=60):
        writer = imageio.get_writer(video_file, fps=fps)

        state_t = state[timestep_from]
        og.sim.load_state(state_t, serialized=True)
        step_and_render()

        for timestep in range(timestep_from, timestep_to + 1):
            state_t = state[timestep]
            og.sim.load_state(state_t, serialized=True)
            # step_and_render()
            og.sim.step()
    ...
    save_timestep_from_to(560, 660, os.path.join(image_folder, "transformed_eef.mp4"), fps=30)
    ...
    save_timestep_from_to(100, 220, os.path.join(image_folder, "base_mp.mp4"), fps=60)
    ...
    save_timestep_from_to(220, 700, os.path.join(image_folder, "arm_mp.mp4"), fps=120)
    ...
    save_timestep_from_to(700, 1000, os.path.join(image_folder, "retract.mp4"), fps=60)
